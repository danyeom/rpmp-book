...
twoway_mixed %>%
  group_by(time) %>%
  t_test(anxiety ~ group, p.adjust.method = "none", detailed = TRUE)
...
twoway_mixed %>%
  group_by(group) %>%
  emmeans_test(anxiety ~ time, p.adjust.method = "none", detailed = TRUE)

emmeans(twoway_mixed_aov, ~ time, by = "group", model = "multivariate") %>%
  pairs(infer = TRUE, adjust = "none")
```
