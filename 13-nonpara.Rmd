...
For posthocs, Jamovi uses Dwass-Steel-Critchlow-Fligner tests (phew!), or simply DCSF tests, for 'post hoc' pairwise comparisons. We'll use the same here for compatibility with Jamovi. The only thing you really need to know about these comparisons is that they have an in-built correction for the family-wise error rate, so do not need adjusting after the analyses have been run.
