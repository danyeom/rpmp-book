# Linear relationships {#sec-regressions-1}
...
```{r echo = FALSE, message = FALSE, fig.dim = c(5, 5), fig.align='center', warning = FALSE}
babies <- read_csv(here("data", "week_10", "w10_babies.csv"))

babies_plot <- babies %>%
  filter_out(case %in% c(261, 870)) %>%   # Remove outliers
  ggplot(
    aes(x = gestation, y = bwt)
  ) +
  geom_point() +
  labs(x = "Gestational period length (Weeks)", y = "Infant birth weight (g)")
...
<details>
    <summary>How is covariance calculated?</summary>
...
```{r echo = FALSE}
data.frame(
  x = c(0.35, -0.24, -0.02, 0.85),
  y = rep("", 4),
  z = rep("", 4)
) %>%
  knitr::kable(
    col.names = c(" ", "Is it positive or negative?", "How strong is it?")
  )
```
...
::: {style="background-color: #cce3c8;  padding: 1.75rem;"}
...
```{r echo = FALSE, warning = FALSE, fig.dim=c(5,5), fig.align='center'}
babies_plot
```
...
```{r warning = FALSE, fig.dim=c(5,5), fig.align='center', echo=FALSE, message=FALSE}
babies_plot +
  geom_smooth(method = "lm", se = FALSE)
```
...
-    **Tolerance** is a related value to VIF - in fact, it is just 1 divided by the VIF - and works in much the same way, except smaller values mean greater collinearity. As a rule of thumb, if tolerance is smaller than 0.20 this suggests an issue.
