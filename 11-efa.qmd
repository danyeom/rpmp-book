...
```{r echo = FALSE}
data.frame(
  msa = c("MSA between .80 - .90", "MSA between .70 - .80", "MSA between .60 - .70", "NSA between .50 - .60", "MSA < .50"),
  interpretation = c("Marvelous", "Middling", "Mediocre", "Miserable", "Unacceptable")
) %>%
...
The Minimum Average Partials (MAP; Velicer, 1976) test is another powerful test that is generally useful at identifying how many factors should be extracted. It basically works by calculating the partial correlations between items and finding their average *after* removing the variance explained by the factors. 

The `EFA.dimensions` package contains a series of helpful functions for running some EFA-related checks. The MAP test is included as part of the `MAP()` function:
