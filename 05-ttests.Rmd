...
```{r}
w8_grades %>% 
  cohens_d(sample ~ 1, mu = 72, ci = TRUE) 
```
For an independent-samples t-test:

```{r}
w8_training %>%
  cohens_d(Years_training ~ Group, ci = TRUE)
```
...
```{r}
w8_symptoms %>%
  cohens_d(symptom_score ~ time, paired = TRUE, ci = TRUE)
```

The Canvas version asks you to interpret each of these effect sizes, but... `rstatix` will automatically label this for you!

Alternatively, you can use the `effectsize` package as follows, although this sometimes isn't all that intuitive:
