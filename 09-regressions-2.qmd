# Regression continued {#sec-regressions-2}
...
In the next section we'll talk about model comparison in a more formal manner. 
...
max(flow_cooksd)
```
If you prefer a tidyverse implementation, you can use the `augment()` function from the `broom` package (see [the appendix](#broom-augment) for more information on `broom`). This returns a whole range of values, including fitted values, residuals (both of which are useful for Q-Q plots) and Cook's distances:
...
### How are Mahalanobis distances calculated?
...
head(mahal)
```
Once we have done that we can leverage the properties of the chi-square distribution to calculate *p*-values for each Mahalanobis distance. Note that df = 3 here, as our original model had 3 predictors.
...
`dplyr` version 1.1.0 though now introduces the `filter_out()` function, which provides a much more readable syntax for filtering *out* rows:

```{r}
flow_no_outliers <- flow_data %>%
  filter_out(id %in% c(378, 528))
```
