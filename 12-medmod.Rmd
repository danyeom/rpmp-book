...
<div style="background-color: #f5f5f5;  padding: 1.75rem;">
No introduction to mediation is complete without considering the causal steps approach, which held dominance as the main method of testing mediations for a while.
</div>
...
One final check - also recall that the total effect, *c*, can be broken down into the direct effect *c'* and the indirect effect *ab*. You can manually check the maths here - *ab* (0.561 * 0.6355) = 0.0396 gives us 0.396, which was our initial estimate of *c*.
...
- `plot = 1` gives us values for plotting the moderation. Unfortunately
...
summary(passion_mod_lm)

interact_plot(passion_mod_lm, pred = "SPANE_pos", modx = "SWLS_TOT")
```
...
moderation_plot1.svg
Lastly, we can examine the Johnson-Neyman plots to see where this relationship is non-significant. We can see that for values of satisfaction with life below ~ -12, the moderation is no longer significant. Note here though, that it's impossible to get a satisfaction with life scale that is negative given the way it is scored; therefore, our main takeaway from this is that the moderation is significant across all possible values of satisfaction with life. Thus, we can say that if you are more satisfied with life, the greater the effect of positive experiences with music on your harmonious passion!